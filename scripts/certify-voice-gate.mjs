...
assert.equal(status.phase, 'VG-03A')
...
assert.equal(typeof status.capabilities.dedicatedWakeEngine, 'boolean')
...
assert.equal(status.capabilities.acousticFrontEnd, true)
assert.equal(status.capabilities.multiMicArbitration, true)
assert.equal(status.capabilities.toolDomainRouter, true)
assert.equal(status.capabilities.capabilityPolicy, true)
assert.equal(status.capabilities.antiReplay, false)

console.log('L.U.M.I.A. Voice Gate VG-03A: PASS')
